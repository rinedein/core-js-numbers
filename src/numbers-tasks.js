...
function toNumber(value, def) {
  ...
  return Number.isFinite(parsedValue) ? parsedValue : def;
}
...
function getSumOfDigits(num) {
  ...
  for (let i = 0; i < convertToString.length; i += 1) {
    sum += parseInt(convertToString[i], 10);
  }
...
function toFixed(number, fractionDigits) {
  const roundedNumber = Number(number.toFixed(fractionDigits));
  const formattedNumber = roundedNumber.toFixed(fractionDigits);
...
function toPrecision(number, precision) {
  const roundedNumber = parseFloat(number.toPrecision(precision));
